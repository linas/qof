...
typedef QofQueryPredData *(*QueryPredicateCopyFunc) (QofQueryPredData *
	pdata);
	...
	const gchar *str, QofStringMatch options, gboolean is_regex)
{
	...
	g_return_val_if_fail (how == QOF_COMPARE_EQUAL
		|| how == QOF_COMPARE_NEQ, NULL);
	...
	QofQueryPredData * pd)
{
	...
	obj_val =
		((query_numeric_getter) getter->param_getfcn) (object, getter);
...
numeric_copy_predicate (QofQueryPredData * pd)
{
	...
	return qof_query_numeric_predicate (pd->how, pdata->options,
		pdata->amount);
}
	...
	QofQueryPredData * pd)
{
	...
	case QOF_GUID_MATCH_ALL:
		...
		for (node = pdata->guids; node; node = node->next)
		{
			...
			{
				guid =
					((query_guid_getter) getter->param_getfcn) (o_list->
					data, getter);
	...
	case QOF_GUID_MATCH_LIST_ANY:
		...
		o_list =
			((query_glist_getter) getter->param_getfcn) (object, getter);
...
{
	gint32 num =
		((query_int32_getter) getter->param_getfcn) (object, getter);
...
{
	gint64 num =
		((query_int64_getter) getter->param_getfcn) (object, getter);
...
{
	double num =
		((query_double_getter) getter->param_getfcn) (object, getter);
...
{
	gboolean num =
		((query_boolean_getter) getter->param_getfcn) (object, getter);
	...
	QofQueryPredData * pd)
{
	...
	case QOF_GUID_MATCH_LIST_ANY:
		{
			o_list =
				((query_glist_getter) getter->param_getfcn) (object,
				getter);
			for (node = o_list; node; node = node->next)
			{
				for (node2 = pdata->guids; node2; node2 = node2->next)
	...
	default:
		{
			guid =
				((query_guid_getter) getter->param_getfcn) (object,
				getter);
			for (node = pdata->guids; node; node = node->next)
			{
				if (guid_equal (node->data, guid))
	...
	QofQueryPredData * pd)
{
	...
	case QOF_GUID_MATCH_ALL:
		...
		for (node = pdata->guids; node; node = node->next)
		{
			...
			{
				guid =
					((query_choice_getter) getter->param_getfcn) (o_list->
					data, getter);
				if (guid_equal (node->data, guid))
					break;
			}
	...
	default:
		...
		guid =
			((query_choice_getter) getter->param_getfcn) (object, getter);
		for (node = pdata->guids; node; node = node->next)
		{
			if (guid_equal (node->data, guid))
				break;
...
qof_query_register_core_object (QofType core_name,
	...
	QueryToString toString, QueryPredicateEqual pred_equal)
{
...
init_tables (void)
{
	...
	} knownTypes[] =
	{
		...
		QOF_TYPE_DEBCRED, numeric_match_predicate,
				...
				numeric_free_pdata, debcred_to_string,
				numeric_predicate_equal},
		{
		QOF_TYPE_NUMERIC, numeric_match_predicate,
				numeric_compare_func, numeric_copy_predicate,
				numeric_free_pdata, numeric_to_string,
				numeric_predicate_equal},
		{
		...
		QOF_TYPE_INT32, int32_match_predicate, int32_compare_func,
				int32_copy_predicate, int32_free_pdata,
				int32_to_string, int32_predicate_equal},
		{
		QOF_TYPE_INT64, int64_match_predicate, int64_compare_func,
				int64_copy_predicate, int64_free_pdata,
				int64_to_string, int64_predicate_equal},
		{
		QOF_TYPE_DOUBLE, double_match_predicate, double_compare_func,
				double_copy_predicate, double_free_pdata,
				double_to_string, double_predicate_equal},
		{
		QOF_TYPE_BOOLEAN, boolean_match_predicate,
				boolean_compare_func, boolean_copy_predicate,
		...
		{
		QOF_TYPE_COLLECT, collect_match_predicate,
				collect_compare_func, collect_copy_predicate,
				collect_free_pdata, NULL, collect_predicate_equal},
		{
	QOF_TYPE_CHOICE, choice_match_predicate, NULL,
				choice_copy_predicate, choice_free_pdata, NULL,
				choice_predicate_equal},};
	...
	for (i = 0; i < (sizeof (knownTypes) / sizeof (*knownTypes)); i++)
	{
		qof_query_register_core_object (knownTypes[i].name,
			...
			knownTypes[i].toString, knownTypes[i].pred_equal);
	}
}
...
gboolean
qof_query_core_predicate_equal (QofQueryPredData * p1,
	QofQueryPredData * p2)
{
	QueryPredicateEqual pred_equal;
	...
	if (p1->how != p2->how)
		return FALSE;
	if (safe_strcmp (p1->type_name, p2->type_name))
		return FALSE;
