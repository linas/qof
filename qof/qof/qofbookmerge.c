...
qof_book_merge_update_rule (QofBookMergeRule * currentRule, gboolean match,
	gint weight)
{
	gboolean absolute;
...
qof_book_merge_compare (QofBookMergeData * mergeData)
{
	...
	while (paramList != NULL)
	{
		...
		{
			kvpImport =
				kvp_frame_copy (qtparam->param_getfcn (mergeEnt, qtparam));
			kvpTarget =
...
static void
qof_book_merge_orphan_check (double difference,
	QofBookMergeRule * mergeRule, QofBookMergeData * mergeData)
{
...
qof_book_merge_match_orphans (QofBookMergeData * mergeData)
{
	...
	while (orphans != NULL)
	{
		...
		if (rule->targetEnt == NULL)
		{
			...
			rule->difference = 0;
			mergeData->mergeList =
				g_list_prepend (mergeData->mergeList, rule);
...
static void
qof_book_merge_foreach_type_target (QofObject * merge_obj,
	gpointer user_data)
{
	QofBookMergeData *mergeData;
	QofBookMergeRule *currentRule;
	...
	if (safe_strcmp (merge_obj->e_type,
			currentRule->importEnt->e_type) == 0)
	{
		qof_object_foreach (currentRule->importEnt->e_type,
			mergeData->targetBook,
...
qof_book_merge_foreach (QofEntity * mergeEnt, gpointer user_data)
{
	...
	*g = mergeEnt->guid;
	...
	targetEnt =
		qof_collection_lookup_entity (qof_book_get_collection
		(mergeData->targetBook, mergeEnt->e_type), g);
	if (targetEnt != NULL)
	{
		mergeRule->mergeAbsolute = TRUE;
		mergeRule->targetEnt = targetEnt;
		g_return_if_fail (qof_book_merge_compare (mergeData) != -1);
		mergeRule->linkedEntList =
			g_slist_copy (currentRule->linkedEntList);
		mergeData->mergeList =
			g_list_prepend (mergeData->mergeList, mergeRule);
		return;
	}
	...
	if (best_matchEnt != NULL)
	{
		...
		g_hash_table_insert (mergeData->target_table, mergeRule->targetEnt,
			mergeRule);
...
qof_book_merge_foreach_param (QofParam * param, gpointer user_data)
{
	...
	if ((param->param_getfcn != NULL) && (param->param_setfcn != NULL))
	{
		mergeData->mergeObjectParams =
			g_slist_append (mergeData->mergeObjectParams, param);
	}
}
...
qof_book_merge_foreach_type (QofObject * merge_obj, gpointer user_data)
{
	...
	if (mergeData->mergeObjectParams != NULL)
		g_slist_free (mergeData->mergeObjectParams);
	mergeData->mergeObjectParams = NULL;
	qof_class_param_foreach (merge_obj->e_type,
		qof_book_merge_foreach_param, mergeData);
	...
	QofBookMergeRule * rule, guint remainder __attribute__ ((unused)))
{
	...
	{
		inst =
			(QofInstance *) qof_object_new_instance (rule->importEnt->
			e_type, mergeData->targetBook);
		g_return_if_fail (inst != NULL);
		rule->targetEnt = &inst->entity;
		qof_entity_set_guid (rule->targetEnt,
			qof_entity_get_guid (rule->importEnt));
	}
	...
	while (rule->mergeParam != NULL)
	{
		...
		{
			boolean_getter =
				(gboolean (*)(QofEntity *, QofParam *)) cm_param->
				param_getfcn;
		...
		{
			cm_kvp =
				kvp_frame_copy (cm_param->
				param_getfcn (rule->importEnt, cm_param));
			kvp_frame_setter =
