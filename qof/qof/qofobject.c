...
void
qof_object_book_begin (QofBook * book)
{
	GList *l;
...
void
qof_object_book_end (QofBook * book)
{
	GList *l;
...
void
qof_object_foreach_type (QofForeachTypeCB cb, gpointer user_data)
{
	GList *l;
...
const gchar *
qof_object_get_type_label (QofIdTypeConst type_name)
{
	const QofObject *obj;
...
void
qof_object_initialize (void)
{
	if (object_is_initialized)
		return;
 ...
 */
gboolean
qof_object_register (const QofObject * object)
{
	g_return_val_if_fail (object_is_initialized, FALSE);

	if (!object)
		return FALSE;
	g_return_val_if_fail (object->interface_version == QOF_OBJECT_VERSION,
		FALSE);

	if (g_list_index (object_modules, (gpointer) object) == -1)
		object_modules =
			g_list_prepend (object_modules, (gpointer) object);
	else
		return FALSE;
...
gboolean
qof_object_register_backend (QofIdTypeConst type_name,
	const gchar * backend_name, gpointer be_data)
{
	GHashTable *ht;
	g_return_val_if_fail (object_is_initialized, FALSE);

	if (!type_name || *type_name == '\0' ||
		!backend_name || *backend_name == '\0' || !be_data)
		return FALSE;
...
gpointer
qof_object_lookup_backend (QofIdTypeConst type_name,
...
static void
foreach_backend (gpointer key, gpointer be_item, gpointer arg)
{
	gchar *data_type = key;
...
void
qof_object_foreach_backend (const gchar * backend_name,
	QofForeachBackendTypeCB cb, gpointer user_data)
{
	GHashTable *ht;
