...
check_unary_op (gboolean (*eqtest) (gnc_numeric, gnc_numeric),
				...
				gnc_numeric actual, gnc_numeric input, const char *errmsg)
{
...
check_binary_op (gnc_numeric expected,
				 gnc_numeric actual,
				 gnc_numeric input_a, gnc_numeric input_b, const char *errmsg)
{
	char *e = gnc_numeric_print (expected);
	char *r = gnc_numeric_print (actual);
...
check_mult_div (void)
{
	...
	b = gnc_numeric_create (-1, 1);
	check_binary_op (gnc_numeric_create (100, 100),
					 gnc_numeric_mul (a, b, GNC_DENOM_AUTO,
									  GNC_HOW_DENOM_EXACT), a, b,
