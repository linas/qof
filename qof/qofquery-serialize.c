...
qof_query_pred_data_to_xml (QofQueryPredData * pd)
{
	...
	if (!safe_strcmp (pd->type_name, QOF_TYPE_STRING))
	{
		...
		PUT_HOW ("qofquery:compare", pd->how, LT, LTE, EQUAL, GT, GTE,
			NEQ);
...
qof_query_sorts_to_xml (QofQuery * q)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		PUT_STR ("qofquery:order",
			increasing ? "DESCENDING" : "ASCENDING");
...
do_qof_query_to_xml (QofQuery * q, xmlNodePtr topnode)
{
	...
	if (terms)
		xmlAddChild (topnode, terms);

	sorts = qof_query_sorts_to_xml (q);
	if (sorts)
		xmlAddChild (topnode, sorts);
...
main (int argc, char *argv[])
{
	...
	qof_sql_query_parse (sq,
		...
		"or not astr=\'asdf\' " "and aint64 = 9876123456789;");
